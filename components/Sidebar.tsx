...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <aside className="hidden md:flex flex-col w-72 bg-white/40 dark:bg-black/40 backdrop-blur-md border-r border-gray-200 dark:border-gray-800 p-4 transition-colors">
      ...
      <button onClick={onNewChat} className="flex items-center gap-2 w-full bg-white/60 dark:bg-gray-800/60 hover:bg-white dark:hover:bg-gray-700 text-gray-900 dark:text-white px-4 py-3 rounded-lg transition-all border border-gray-200 dark:border-gray-700 mb-6 group shadow-sm hover:shadow-md">
      ...
      <div className="flex-1 overflow-y-auto -mx-2 px-2 mb-4 scrollbar-hide">
        ...
        <div className="space-y-1">
          {sessions.map(session => (
            <div key={session.id} onClick={() => onSelectSession(session.id)} className={`group flex items-center justify-between p-2 rounded-lg cursor-pointer transition-colors text-sm ${activeSessionId === session.id ? 'bg-gray-200/80 dark:bg-gray-800/80 text-gray-900 dark:text-white font-medium shadow-sm' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100/50 dark:hover:bg-gray-800/30 hover:text-gray-900 dark:hover:text-gray-200'}`}>
              ...
              <button onClick={(e) => onDeleteSession(e, session.id)} className="opacity-0 group-hover:opacity-100 p-1 hover:bg-red-100 dark:hover:bg-red-900/30 text-gray-400 hover:text-red-500 rounded transition-all"><Trash2 className="w-3.5 h-3.5" /></button>
            </div>
          ))}
      ...
      <div className="flex-shrink-0 border-t border-gray-200 dark:border-gray-800 pt-4 space-y-3">
        ...
        <button onClick={onOpenSettings} className="flex items-center justify-center gap-2 w-full px-3 py-2 text-sm font-medium bg-white/50 dark:bg-gray-800/50 hover:bg-white dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-lg border border-gray-200 dark:border-gray-700 transition-colors shadow-sm"><Settings className="w-4 h-4" /><span>{t('action.settings', language)}</span></button>
      </div>
    </aside>
  );
};
